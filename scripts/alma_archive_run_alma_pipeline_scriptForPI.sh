...
if [[ ! -f "$HOME/Softwares/CASA/Portable/bin/bin_setup.bash" ]] && [[ ! -f "$HOME/Software/CASA/Portable/bin/bin_setup.bash" ]]; then
    echo "Error! \"$HOME/Software/CASA/Portable/bin/binsetup.bash\" was not found!"
...
check_and_extract_casa_version_in_readme_file() {
    if [[ $# -ge 1 ]]; then
		...
		## if no README file or failed to extract CASA Version from there, then we read "qa/*.tgz"
                echo "********start searching for CASA version!*********"
                if [[ -d "$script_dir/qa" ]] || [[ -L "$script_dir/qa" ]]; then
                    list_of_found_files=()
                    script_finding_casa_version=""
                    echo "start searching in weblog"
                    ...
                    if [[ ${#list_of_found_files[@]} -eq 0 ]]; then
                        list_of_found_files=($(find -L "$script_dir/qa" -name "member*.html"))
                        script_finding_casa_version=alma_archive_find_casa_version_in_qa_html.py
                    fi
                    echo "start searching in scriptForCalibration"
                    if [[ ${#list_of_found_files[@]} -eq 0 ]]; then
        ...
        if [[ -d "$script_dir/calibrated" ]] || [[ -L "$script_dir/calibrated" ]]; then
            ...
            if [[ -d "$script_dir/calibrated/calibrated_final.ms" ]] || [[ -L "$script_dir/calibrated/calibrated_final.ms" ]]; then
                ...
                if [[ -d "$script_dir/calibrated/calibrated.ms" ]] || [[ -L "$script_dir/calibrated/calibrated.ms" ]]; then
                    ...
                    list_of_lz4_files=($(find "$script_dir/calibrated/" -mindepth 1 -maxdepth 1 -type f -name "dlv_*.tar.lz4"))
                    # if "calibrated" dir exists, but no "calibrated*.ms", check if ".tar.lz4" files exist or not
                    if [[ ${#list_of_lz4_files[@]} -gt 0 ]]; then
                        echo "Found \"dlv_*.tar.lz4\", will unpack it first!!"
                        for file in $list_of_lz4_files; do
                            lz4 -d "$file" | tar xv -C "$script_dir/calibrated/"
                            mv "$file" "$script_dir/"
                        done
                        echo "have unpacked all lz4 files"
                        return 2
                    fi
...
current_dir=$(pwd)
echo "current direcotry is \"${current_dir}\""
...
for (( i = 0; i < ${#list_of_input_dirs[@]}; i++ )); do
    ...
    echo "The input direcotry is \"${input_dir}\"！"
    # 
    ...
    if [[ ${#list_of_script_files[@]} -eq 0 ]]; then
        ...
        echo "This is the calibrated ms received from ALMA help desk, try check_and_concat_calibrated_ms directly"
        casa_version="CASA version 5.1.1"
        echo "$casa_version" > "$input_dir/README_CASA_VERSION"
        check_and_concat_calibrated_ms "$input_dir"
        check_return_code=$?
        if [[ $check_return_code -eq 255 ]]; then
            echo "check_and_concat_calibrated_ms FAILED!"
            exit 255 # got error when running the function, exit
        elif [[ $check_return_code -eq 2 ]]; then
            echo "check the directory, probably unpack the lz4 file?"
            exit 2
        else
            echo "check_and_concat_calibrated_ms done?"
        fi
    fi
    # 
