...
for (( i = 0; i < ${#list_of_datasets[@]}; i++ )); do
    ...
    if [[ $check_return_code -eq 255 ]]; then
        echo_error "check_and_concat_calibrated_ms FAILED!"
    elif [[ $check_return_code -eq 2 ]]; then
        echo_error "Unpacked the lz4 file! Need to check the directory/files it produce!!"
    else
        echo_output "check_and_concat_calibrated_ms done successfully"
    fi
    # check output
